...
class EditLog extends StatefulWidget {
  ...
  const EditLog({super.key, required this.logData, required this.logId});
...
class _EditLogState extends State<EditLog> {
  ...
  void initState() {
    super.initState();
    _initializeFromLogData();
    _loadData();
  }

  void _initializeFromLogData() {
    // Initialize form fields with existing log data
    _selectedLocation = widget.logData['location'] as String?;
    _selectedWorkType = widget.logData['workType'] as String?;
    _selectedWorkers = List<String>.from(widget.logData['workers'] ?? []);
    
    // Parse work date
    if (widget.logData['workDate'] is Timestamp) {
      _workDate = (widget.logData['workDate'] as Timestamp).toDate();
    }
    
    // Parse times
    final startTimeStr = widget.logData['startTime'] as String?;
    final endTimeStr = widget.logData['endTime'] as String?;
    ...
    // Set text fields
    _summaryController.text = widget.logData['summary'] as String? ?? '';
    _descriptionController.text = widget.logData['description'] as String? ?? '';
    
    // Copy existing photo URLs
    final photos = widget.logData['photos'] as List? ?? [];
    _existingPhotoUrls.addAll(photos.cast<String>());
  }
  ...
  void _removeExistingPhoto(int index) => setState(() => _existingPhotoUrls.removeAt(index));
  ...
  Future<void> _updateLog() async {
    final confirm = await showDialog<bool>(
      ...
      builder:
          (_) => AlertDialog(
            ...
            content: const Text(
              'Are you sure you want to update this work log?',
            ),
            actions: [
              TextButton(
    ...
    try {
      ...
      if (!mounted) return;
      Navigator.pop(currentContext, true); // Return true to indicate update
      ScaffoldMessenger.of(currentContext).showSnackBar(
        const SnackBar(content: Text('Work log updated successfully!')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(
            ...
            children: [
              ...
              if (_selectedWorkType == 'Other') ...[
                ...
                TextFormField(
                  ...
                  maxLines: null,
                  initialValue: _otherWorkTypeDescription,
              ...
              if (_existingPhotoUrls.isNotEmpty) ...[
                ...
                Wrap(
                  spacing: 10,
                  runSpacing: 10,
                  children: List.generate(_existingPhotoUrls.length, (i) {
                    return Stack(
